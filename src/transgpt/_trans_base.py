...
class BaseTranslation :
    ...
    def __init__(self, api_url, api_id, api_key, args={}) -> None :
    ...
    @abstractmethod
    def translate(self, data_seg) -> str :
        return ""
...
def _machine_translate(client, data) :
    ...
    for seg in segs :
        ...
        with open('./py/itemInfo.lub', "a+", encoding=CHARSET) as file :
            file.write(trans_seg)
        log.info("翻译完成，译文已追加存储到 [itemInfo]")
        time.sleep(1)
...
class ChatgptTranslation :
    ...
    def __init__(self, openai_model, openai_key, proxy_ip, proxy_port) :
        ...
        self.role_setting = {"role": "system", "content": "基于网游 仙境传说 ragnarok online 的背景，把韩文内容翻译成中文，并润色。不要说多余的内容。"}
